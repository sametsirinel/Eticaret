<?php
	...
	class sepet extends Controller{
		...
		public function alisverisitamamla(){
			...
			while(true){
				...
				if($this->sepet_model->siparisKontrol($session_id)->totalRows()<1)
					break;

			}

			$postlar["session_id"] = $session_id;
			$postlar["userid"] = User::id();

			if(!$this->sepet_model->siparisEkle($postlar))
				Warning::set("Sipariş Oluşturulurken Bir Sorunla Karşılaştık","info");

			$siparisId = DB::insertId();

			foreach(Cart::selectItems() as $item){

				if($this->sepet_model->urunKontrol($item["id"])->totalRows()>0){

					$urun["urunid"] = $item["id"];
					$urun["adet"] = $item["quantity"];
					$urun["ozellikid"] = $item["fiyatid"];
					$urun["siparisid"] = $siparisId;

					$this->sepet_model->sepeteEkle($urun);

				}

			}

			redirect(baseurl("sepet/odemesecimi/$siparisId"));
