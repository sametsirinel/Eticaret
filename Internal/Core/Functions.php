...
function internalRouteURI($requestUri = '')
{
	...
	$patternType	  = $config['patternType'];
 	$showBadRequest = $config['showBadRequest'];

	if( ! empty($uriChange) ) foreach( $uriChange as $key => $val )
	{
		$method = is_array($val) ? (isset($val["method"]) ? (strtolower($val["method"])=="post" ? "post" : "get" ) : "get") : null;
		$url = is_array($val) ? (isset($val["url"]) ? $val["url"] : $showBadRequest) : null;

		if( $patternType === 'classic' )
		{
			if(is_array($val)){
				
				if(!(preg_match(presuffix($key).'xi', $requestUri) && Method::$method())){
					$requestUri = preg_replace(presuffix($key).'xi',$url, $requestUri);
				}
